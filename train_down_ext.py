...
save_dir = "./weight/mandi_ext/PNG+mask_final_modi_real/checkpoint_model_{}_lr_{}_batchsize_{}".format(args.model_name, args.learning_rate, args.batch_size)
#save_dir = "./weight/mandi_ext/PNG+mask_final/checkpoint_model_{}_lr_{}_batchsize_{}".format(args.model_name, args.learning_rate, args.batch_size)
#save_dir = "./weight/mandi_ext/PNG_segmask/checkpoint_model_{}_lr_{}_batchsize_{}".format(args.model_name, args.learning_rate, args.batch_size)
...
train_dir = []
test_dir = []
train_imgs = glob.glob("./dataset/crop_PNG_Images/train_man/*.png")
val_imgs = glob.glob("./dataset/crop_PNG_Images/val_man/*.png")
...
train_loader = torch.utils.data.DataLoader(train_dataset, sampler=ImbalancedDatasetSampler(train_dataset), batch_size = args.batch_size ,shuffle=False, num_workers=4, drop_last=True)
...
#     model.load_from(np.load(args.pretrained_dir))
    ...
    model = timm.models.resnet.resnet50d(pretrained=True, num_classes=4)
    ...
    model = timm.models.efficientnet.efficientnet_l2(pretrained=True, num_classes=4)
elif args.model == 'coat':
    model = timm.models.coat.coat_lite_small(pretrained=True, num_classes=4)
...
for epoch in range(args.epoch_num):
    train_total = 0
    train_correct = 0
    confusion_matrix_train = torch.zeros(4, 4)
